...
def generator(_x, _rand_dim, _batch_size, reuse=False):
    with tf.name_scope("generator"):
        ...
        g_1 = tflayers.conv2d(g_0, 30, 11, 1, activation_fn=tf.nn.leaky_relu, normalizer_fn=tflayers.batch_norm, reuse=reuse, scope="gen_1")
        g_2 = tflayers.conv2d(g_1, 30, 11, 1, activation_fn=tf.nn.leaky_relu, normalizer_fn=tflayers.batch_norm, reuse=reuse, scope="gen_2")
        g_3 = tflayers.conv2d(g_2, 5, 11, 1, activation_fn=None, normalizer_fn=None, reuse=reuse, scope="gen_4")
        return g_3
...
def discriminator(_origin, _marking_onehot, _batch_size, reuse=False):
    with tf.name_scope("discriminator"):
        ...
        d_1 = tflayers.conv2d(d_0, 16, 15, 4, activation_fn=tf.nn.leaky_relu, normalizer_fn=tflayers.batch_norm, reuse=reuse, scope="dis_1")  # 128 -> 32
        d_2 = tflayers.conv2d(d_1, 32, 7, 2, activation_fn=tf.nn.leaky_relu, normalizer_fn=tflayers.batch_norm, reuse=reuse, scope="dis_2")  # 32 -> 16
        d_3 = tflayers.conv2d(d_2, 64, 5, 2, activation_fn=tf.nn.leaky_relu, normalizer_fn=tflayers.batch_norm, reuse=reuse, scope="dis_3")  # 16 -> 8
        d_4 = tflayers.conv2d(d_3, 128, 3, 2, activation_fn=tf.nn.leaky_relu, normalizer_fn=tflayers.batch_norm, reuse=reuse, scope="dis_4")  # 8 -> 4
        d_5 = tflayers.fully_connected(tf.reshape(d_4, [_batch_size, 128 * 4 * 4]), 512, activation_fn=tf.nn.tanh, normalizer_fn=tflayers.batch_norm, reuse=reuse, scope="dis_5")
        d_6 = tflayers.fully_connected(d_5, 1, activation_fn=None, normalizer_fn=None, reuse=reuse, scope="dis_6")
        return d_6
...
def model(_origin_uint8, _marking, _batch_size, _rand_dim):
    ...
    return _dis_loss, _gen_loss, _loss, _accuracy
