...
def model(x, y, _batch_size, _image_size):
    ...
    h0_3 = conv2d(tf.image.resize_images(h0_2, [_image_size, _image_size]), 1, k_h=9, k_w=9, strides=(1, 1, 1, 1), name="h0_conv2d_3")
    h0_4 = tf.nn.sigmoid(h0_3, name="h0_sigmoid_4")
    y_0 = tf.cast(tf.reshape(tf.equal(y, 0), (_batch_size, _image_size, _image_size, 1)), dtype=tf.float32)
    ...
    h1_3 = conv2d(tf.image.resize_images(h1_2, [_image_size, _image_size]), 1, k_h=9, k_w=9, strides=(1, 1, 1, 1), name="h1_conv2d_3")
    h1_4 = tf.nn.sigmoid(h1_3, name="h1_sigmoid_4")
    ...
    h2_3 = conv2d(tf.image.resize_images(h2_2, [_image_size, _image_size]), 1, k_h=9, k_w=9, strides=(1, 1, 1, 1), name="h2_conv2d_3")
    h2_4 = tf.nn.sigmoid(h2_3, name="h2_sigmoid_4")
    y_2 = tf.cast(tf.reshape(tf.equal(y, 2), (_batch_size, _image_size, _image_size, 1)), dtype=tf.float32)
    ...
    h3_3 = conv2d(tf.image.resize_images(h3_2, [_image_size, _image_size]), 1, k_h=9, k_w=9, strides=(1, 1, 1, 1), name="h3_conv2d_3")
    h3_4 = tf.nn.sigmoid(h3_3, name="h3_sigmoid_4")
    y_3 = tf.cast(tf.reshape(tf.equal(y, 3), (_batch_size, _image_size, _image_size, 1)), dtype=tf.float32)
    ...
    h4_3 = conv2d(tf.image.resize_images(h4_2, [_image_size, _image_size]), 1, k_h=9, k_w=9, strides=(1, 1, 1, 1), name="h4_conv2d_3")
    h4_4 = tf.nn.sigmoid(h4_3, name="h4_sigmoid_4")
    y_4 = tf.cast(tf.reshape(tf.equal(y, 4), (_batch_size, _image_size, _image_size, 1)), dtype=tf.float32)
    loss_4 = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=y_4, logits=h4_3, name="loss_3"))
    # 汇总
    h_4 = tf.concat([h0_4, h1_4, h2_4, h3_4, h4_4], axis=3, name="concat_4")
    loss_total = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=tf.one_hot(y, 5), logits=h_4))
